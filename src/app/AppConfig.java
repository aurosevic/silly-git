...
public class AppConfig {
    ...
    public static void readConfig(String configName, int serventId) {
        ...
        try {
            serventRoot = properties.getProperty("servent" + serventId + ".root");
        } catch (Exception e) {
            timestampedErrorPrint("Property root doesn't exist for servent with id " + serventId + ".");
            System.exit(0);
        }

        String serventStorage = "";

        try {
            serventStorage = properties.getProperty("servent" + serventId + ".storage");
        } catch (Exception e) {
            timestampedErrorPrint("Property storage doesn't exist for servent with id " + serventId + ".");
            System.exit(0);
        }

        myServentInfo = new ServentInfo("localhost", serventPort, serventRoot, serventStorage);
    }
