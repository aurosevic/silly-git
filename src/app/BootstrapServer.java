...
public class BootstrapServer {
    ...
    public void doBootstrap(String bsIp, int bsPort) {
        ...
        try {
            listenerSocket = new ServerSocket(bsPort, 50, InetAddress.getByName(bsIp));
    ...
    public static void main(String[] args) {
        ...
        int bsPort = 0;
        try {
            bsPort = Integer.parseInt(args[1]);
        } catch (NumberFormatException e) {
            AppConfig.timestampedErrorPrint("Bootstrap port not valid: " + args[0]);
            System.exit(0);
        }

        AppConfig.timestampedStandardPrint("Bootstrap server started on ip: [" + bsIp + "] and port: " + bsPort);

        BootstrapServer bs = new BootstrapServer();
        bs.doBootstrap(bsIp, bsPort);
    }
}
